                ...
                while (runnable) {
                    try {
                        ...
                        if (timeNow.compareTo(histTimeStart) >= 0
                             ...
                             * 
                             * for (int k = 0; k < 2; k++) { LOGGER.info(new Date().toString() +
                             * ">>>>>>>>>>>>>>>>>>>>>"); final List<Instrument> instrument =
                             * instrumentList.subList(size*k, size*(k+1)-1); LOGGER.info(new
                             * Date().toString() + ">>>outer loop>>> " + k + " " +
                             * instrument.size());
                             * 
                             * LOGGER.info(new Date().toString() + ">>>InThread>>>" +
    ...
    private void applySmaStragegy() {
        ...
        if (null != instrumentList && instrumentList.size() > 0) {
            ...
            for (int i = 0; i < instrumentList.size(); i++) {
                ...
                if (null == quoteData || quoteData.size() < 26)
                    continue;
