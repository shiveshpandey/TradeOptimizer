...
public class TradeOptimizer {
					...
					try {
						...
						if (timeNow.compareTo(timeStart) >= 0 && timeNow.compareTo(timeEnd) <= 0) {
							...
							if (firstTimeDayHistoryRun) {
								try {
									instrumentList = tradeOperations.getInstrumentsForExchange(kiteconnect, "BFO");
							...
							for (int index = 0; index < instrumentList.size(); index++) {
								try {
									...
									for (int count = 0; count < historicalData.dataArrayList.size(); count++) {
										...
										quote = new StreamingQuoteModeQuote(dtFmt.format(
												fmt.parse(historicalData.dataArrayList.get(count).timeStamp).getTime()),
	...
	private void applySmaStragegy() {
		...
		for (int i = 0; i < instrumentList.size(); i++) {
			quoteData = streamingQuoteStorage.getProcessableQuoteDataOnTokenId(instrumentList.get(i).toString(), 26);
	...
	private void startStreamingQuote(String apiKey, String userId, String publicToken) {
		// String URIstring = StreamingConfig.STREAMING_QUOTE_WS_URL_TEMPLATE +
		...
		if (null != instrumentList) {
			try {
				 ...
				 * websocketThread = new WebsocketThread(URIstring,
				 * instrumentList, streamingQuoteStorage); streamingQuoteStarted
				 * = websocketThread.startWS(); if (streamingQuoteStarted) {
				 * Thread t = new Thread(websocketThread); t.start(); } else {
				 * System.out.println(
