...
public class TradeOptimizer {
            ...
            try {
                ...
                for (int i = 0; i < arr.length(); i++)
                    if (!stocksSymbolArray.containsKey(arr.getJSONObject(i).getString("symbol")))
                        stocksSymbolArray.put(arr.getJSONObject(i).getString("symbol"), (Math
                                .abs(Double.valueOf(arr.getJSONObject(i).getString("netPrice")))
                                * (Double.valueOf(arr.getJSONObject(i).getString("tradedQuantity")
                                        .replaceAll(",", "")) / 100000)));
            } catch (IOException e) {
                LOGGER.info(e.getMessage());
            }
    ...
    private void threadEnabledHistoricalDataFetch(List<Instrument> instrumentList) {
        ...
        for (int index = 0; index < instrumentList.size(); index++) {
            try {
                ...
                priorityPoint = 0;
                historicalDataList = tradeOperations.getHistoricalData(kiteconnect,
                        histDataStreamStartDateTime, histDataStreamEndDateTime, "minute",
                        Long.toString(instrument.getInstrument_token()));
                ...
                for (int count = 0; count < historicalDataList.dataArrayList.size(); count++) {
                    ...
                    quote = new StreamingQuoteModeQuote(
                            historicalDataList.dataArrayList.get(count).timeStamp,
                ...
                while (runnable) {
                    try {
                        ...
                        if (timeNow.compareTo(histTimeStart) >= 0
                            ...
                            histDataStreamStartDateTime = todaysDate + " "
    ...
    private void roundOfNonPerformingBoughtStocks(ArrayList<Long> quoteStreamingInstrumentsArr,
            ArrayList<Long> previousQuoteStreamingInstrumentsArr, KiteConnect kiteconnect)
        ...
        for (int index = 0; index < previousQuoteStreamingInstrumentsArr.size(); index++) {
            ...
            for (int count = 0; count < quoteStreamingInstrumentsArr.size(); count++) {
                if (quoteStreamingInstrumentsArr.get(count)
                        .longValue() == previousQuoteStreamingInstrumentsArr.get(index)
                                .longValue()) {
        ...
        for (int index = 0; index < quoteStreamingInstrumentsArr.size(); index++) {
            ...
            for (int count = 0; count < previousQuoteStreamingInstrumentsArr.size(); count++) {
                if (quoteStreamingInstrumentsArr.get(index)
                        .longValue() == previousQuoteStreamingInstrumentsArr.get(count)
                                .longValue()) {
                    newQuote = false;
                }
            }
                    ...
                    while (runnable) {
                        ...
                        if (timeNow.compareTo(timeStart) >= 0 && timeNow.compareTo(timeEnd) <= 0) {
                            ...
                            if (null != instrumentList && instrumentList.size() > 0) {
                                ...
                                streamingQuoteStorage.saveGeneratedSignals(signalList,
                                        instrumentList);
                            }
