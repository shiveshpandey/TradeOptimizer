...
public class TradeOptimizer {
                ...
                try {
                    ...
                    if (!firstLine) {
                        ...
                        instrumentVolatilityScore
                                .setDailyVolatility(Double.parseDouble(readData[6]) * 100.0);
                        instrumentVolatilityScore
                                .setAnnualVolatility(Double.parseDouble(readData[7]) * 100.0);
    ...
    private void tickerSettingInitialization() {
        ...
        tickerProvider.setOnConnectedListener(new OnConnect() {
            ...
            public void onConnected() {
                if (null != tokenListForTick && tickerProvider.getSubscribedTokenList()
                        .size() != tokenListForTick.size())
                    try {
        ...
        if (streamingQuoteStorage != null) {
            ...
            try {
                streamingQuoteStorage.createDaysStreamingQuoteTable(
                        quoteTableDtFmt.format(Calendar.getInstance().getTime()));
    ...
    private void startStreamingQuote() {
        ...
        if (null != tokenListForTick && tokenListForTick.size() > 0) {
            try {
                if (liveStreamFirstRun) {
                    if (!tickerStarted || (null != tickerProvider
                            && tickerProvider.getSubscribedTokenList().size() == 0
