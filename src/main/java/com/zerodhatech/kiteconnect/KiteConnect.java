...
/**
 * Offers all the functionality like placing order, fetch margins, orderbook,
 * positions, fetch market snap quote.
 */
public class KiteConnect {
	 ...
	 * 
	 * @param apiKey
	 *            is the api key provided after creating new Kite Connect app on
	 *            developers console.
	 */
	 ...
	 * 
	 * @param hook
	 *            can be set to get callback when session is expired.
	 */
	 ...
	 * 
	 * @param enable
	 *            is set to true to enable logging.
	 */
	 ...
	 * 
	 * @param proxy
	 *            defined by user for making requests.
	 */
	 ...
	 * 
	 * @param accessToken
	 *            is the access token received after sending request token and
	 *            api secret.
	 */
	 ...
	 * 
	 * @param publicToken
	 *            is the public token received after sending request token and
	 *            api secret.
	 */
	 ...
	 * 
	 * @param requestToken
	 *            received from login process.
	 * @param apiSecret
	 *            which is unique for each aap.
	 * @return User is the user model which contains user and session details.
	 * @throws KiteException
	 *             is thrown for all Kite trade related errors.
	 * @throws JSONException
	 *             is thrown when there is exception while parsing response.
	 * @throws IOException
	 *             is thrown when there is connection error.
	 */
	public User generateSession(String requestToken, String apiSecret)
		...
		return new User().parseResponse(
				new KiteRequestHandler(proxy).postRequest(routes.get("api.validate"), params, apiKey, accessToken));
	}
	 ...
	 * 
	 * @param refreshToken
	 *            is the refresh token obtained after generateSession.
	 * @param apiSecret
	...
	public TokenSet renewAccessToken(String refreshToken, String apiSecret) throws IOException, KiteException {
		...
		JSONObject response = new KiteRequestHandler(proxy).postRequest(routes.get("api.refresh"), params, apiKey,
				accessToken);
	 ...
	 * @param str
	 *            is the String that has to be encrypted.
	 */
	 ...
	 * 
	 * @return Map of String and Margin is a map of commodity or equity string
	 *         and funds data.
	 * @throws KiteException
	 *             is thrown for all Kite trade related errors.
	 * @throws JSONException
	 ...
	 *            is Order params.
	 * @param variety
	 *            variety="regular". Order variety can be bo, co, amo, regular.
	...
	public Order placeOrder(OrderParams orderParams, String variety) throws KiteException, JSONException, IOException {
		...
		if (orderParams.transactionType != null)
			params.put("transaction_type", orderParams.transactionType);
		...
		if (orderParams.product != null)
			params.put("product", orderParams.product);
		if (orderParams.orderType != null)
			params.put("order_type", orderParams.orderType);
		if (orderParams.validity != null)
			params.put("validity", orderParams.validity);
		if (orderParams.disclosedQuantity != null)
			params.put("disclosed_quantity", orderParams.disclosedQuantity);
		if (orderParams.triggerPrice != null)
			params.put("trigger_price", orderParams.triggerPrice);
		...
		if (orderParams.trailingStoploss != null)
			params.put("trailing_stoploss", orderParams.trailingStoploss);
		if (orderParams.tag != null)
			params.put("tag", orderParams.tag);
	 ...
	 *            variety="regular". Order variety can be bo, co, amo, regular.
	 * @param orderId
	 *            order id of the order being modified.
	...
	public Order modifyOrder(String orderId, OrderParams orderParams, String variety)
		...
		if (orderParams.trailingStoploss != null)
			params.put("trailing_stoploss", orderParams.trailingStoploss);
		if (orderParams.parentOrderId != null)
			params.put("parent_order_id", orderParams.parentOrderId);
	 ...
	 * @param variety
	 *            [variety="regular"]. Order variety can be bo, co, amo,
	 *            regular.
	 * @return Order object contains only orderId.
	 * @throws KiteException
	 *             is thrown for all Kite trade related errors.
	 ...
	 *             is thrown for all Kite trade related errors.
	 * @param orderId
	 *            is the order id which is obtained from orderbook.
	 * @throws KiteException
	 *             is thrown for all Kite trade related errors.
	 * @throws IOException
	 *             is thrown when there is connection error.
	 ...
	 * 
	 * @param orderId
	 *            order if of the order whose trades are fetched.
	...
	public List<Trade> getOrderTrades(String orderId) throws KiteException, JSONException, IOException {
		Map<String, Object> params = new HashMap<String, Object>();
		JSONObject response = new KiteRequestHandler(proxy)
				.getRequest(routes.get("orders.trades").replace(":order_id", orderId), apiKey, accessToken);
		return Arrays.asList(gson.fromJson(String.valueOf(response.get("data")), Trade[].class));
	}
	...
	public List<Holding> getHoldings() throws KiteException, JSONException, IOException {
		Map<String, Object> params = new HashMap<>();
		JSONObject response = new KiteRequestHandler(proxy).getRequest(routes.get("portfolio.holdings"), apiKey,
				accessToken);
	...
	public Map<String, List<Position>> getPositions() throws KiteException, JSONException, IOException {
		...
		JSONObject response = new KiteRequestHandler(proxy).getRequest(routes.get("portfolio.positions"), apiKey,
				accessToken);
		JSONObject allPositions = response.getJSONObject("data");
		positionsMap.put("net",
				Arrays.asList(gson.fromJson(String.valueOf(allPositions.get("net")), Position[].class)));
		positionsMap.put("day",
				Arrays.asList(gson.fromJson(String.valueOf(allPositions.get("day")), Position[].class)));
		return positionsMap;
	}

	/**
	 * Modifies an open position's product type. Only an MIS, CNC, and NRML
	 * positions can be converted.
	 * 
	 * @param tradingSymbol
	 *            Tradingsymbol of the instrument (ex. RELIANCE, INFY).
	 * @param exchange
	 *            Exchange in which instrument is listed (NSE, BSE, NFO, BFO,
	 *            CDS, MCX).
	 * @param transactionType
	 *            Transaction type (BUY or SELL).
	 * @param positionType
	 *            day or overnight position
	 * @param oldProduct
	...
	public JSONObject convertPosition(String tradingSymbol, String exchange, String transactionType,
			String positionType, String oldProduct, String newProduct, int quantity)
			throws KiteException, JSONException, IOException {
		Map<String, Object> params = new HashMap<>();
	 ...
	 *
	 * Response is array for objects. For example, { instrument_token:
	 * '131098372', exchange_token: '512103', tradingsymbol: 'NIDHGRN', name:
	 * 'NIDHI GRANITES', last_price: '0.0', expiry: '', strike: '0.0',
	 * tick_size: '0.05', lot_size: '1', instrument_type: 'EQ', segment: 'BSE',
	 * exchange: 'BSE' }, ...]
	 * 
	 ...
	 *             is thrown for all Kite trade related errors.
	 * @throws IOException
	 *             is thrown when there is connection related errors.
	 */
	 ...
	 * 
	 * @param exchange
	 *            Filter instruments based on exchange. exchange can be NSE,
	 *            BSE, NFO, BFO, CDS, MCX.
	 ...
	 *             is thrown when there is exception while parsing response.
	 * @throws IOException
	 *             is thrown when there is connection related error.
	 */
	 ...
	 *
	 * @param instruments
	 *            is the array of tradingsymbol and exchange or instrument
	 *            token. For example {NSE:NIFTY 50, BSE:SENSEX} or {256265, 265}
	 *
	...
	public Map<String, Quote> getQuote(String[] instruments) throws KiteException, JSONException, IOException {
		KiteRequestHandler kiteRequestHandler = new KiteRequestHandler(proxy);
		JSONObject jsonObject = kiteRequestHandler.getRequest(routes.get("market.quote"), "i", instruments, apiKey,
				accessToken);
	...
	/**
	 * Retrieves OHLC and last price. User can either pass exchange with
	 * tradingsymbol or instrument token only. For example {NSE:NIFTY 50,
	 * BSE:SENSEX} or {256265, 265}
	 * 
	 * @return Map of String and OHLCQuote.
	 * @param instruments
	 *            is the array of tradingsymbol and exchange or instruments
	 *            token.
	 * @throws KiteException
	 *             is thrown for all Kite trade related errors.
	 * @throws IOException
	...
	public Map<String, OHLCQuote> getOHLC(String[] instruments) throws KiteException, IOException {
		JSONObject resp = new KiteRequestHandler(proxy).getRequest(routes.get("quote.ohlc"), "i", instruments, apiKey,
				accessToken);
	...
	public Map<String, LTPQuote> getLTP(String[] instruments) throws KiteException, IOException {
		JSONObject response = new KiteRequestHandler(proxy).getRequest(routes.get("quote.ltp"), "i", instruments,
				apiKey, accessToken);
	 ...
	 *             is thrown when there is connection related error.
	 */
	public TriggerRange getTriggerRange(String exchange, String tradingSymbol, String trasactionType)
			throws KiteException, JSONException, IOException {
		Map<String, Object> params = new HashMap<>();
		params.put("transaction_type", trasactionType);

		String url = routes.get("market.trigger_range").replace(":exchange", exchange).replace(":tradingsymbol",
				tradingSymbol);
	 ...
	 * 
	 * @param from
	 *            "yyyy-mm-dd" for fetching candles between days and "yyyy-mm-dd
	 *            hh:mm:ss" for fetching candles between timestamps.
	 * @param to
	 *            "yyyy-mm-dd" for fetching candles between days and "yyyy-mm-dd
	 *            hh:mm:ss" for fetching candles between timestamps.
	 * @param continuous
	 *            set to true for fetching continuous data of expired
	 *            instruments.
	 * @param interval
	 *            can be minute, day, 3minute, 5minute, 10minute, 15minute,
	 *            30minute, 60minute.
	 * @param token
	 *            is instruments token.
	 * @return HistoricalData object which contains list of historical data
	 *         termed as dataArrayList.
	 * @throws KiteException
	 *             is thrown for all Kite trade related errors.
	 * @throws IOException
	 *             is thrown when there is connection related error.
	 ...
	 * @param quantity
	 *            Quantity to SELL. Not applicable on BUYs. If the holding is
	 *            less than minimum_redemption_quantity, all the units have to
	 *            be sold.
	 * @param tag
	 ...
	 *             is thrown when there is connection related error.
	 */
	public MFOrder placeMFOrder(String tradingsymbol, String transactionType, double amount, double quantity,
			String tag) throws KiteException, IOException {
		Map<String, Object> params = new HashMap<String, Object>();
		params.put("tradingsymbol", tradingsymbol);
		params.put("transaction_type", transactionType);
		params.put("amount", amount);
		if (transactionType.equals(Constants.TRANSACTION_TYPE_SELL))
			params.put("quantity", quantity);
		params.put("tag", tag);

		JSONObject response = new KiteRequestHandler(proxy).postRequest(routes.get("mutualfunds.orders.place"), params,
				apiKey, accessToken);
	...
	/**
	 * If cancel is successful then api will respond as 200 and send back true
	 * else it will be sent back to user as KiteException.
	 * 
	 * @return true if api call is successful.
	 * @param orderId
	 *            is the order id of the mutualfunds order.
	 * @throws KiteException
	 *             is thrown for all Kite trade related errors.
	 * @throws IOException
	 *             is thrown when there connection related error.
	 */
	public boolean cancelMFOrder(String orderId) throws KiteException, IOException {
		KiteRequestHandler kiteRequestHandler = new KiteRequestHandler(proxy);
		kiteRequestHandler.deleteRequest(routes.get("mutualfunds.cancel_order").replace(":order_id", orderId),
				new HashMap<String, Object>(), apiKey, accessToken);
		return true;
	}
	...
	public List<MFOrder> getMFOrders() throws KiteException, IOException {
		JSONObject response = new KiteRequestHandler(proxy).getRequest(routes.get("mutualfunds.orders"), apiKey,
				accessToken);
	 ...
	 * 
	 * @param orderId
	 *            is the order id of a mutualfunds scrip.
	...
	public MFOrder getMFOrder(String orderId) throws KiteException, IOException {
		JSONObject response = new KiteRequestHandler(proxy)
				.getRequest(routes.get("mutualfunds.order").replace(":order_id", orderId), apiKey, accessToken);
	 ...
	 * @param amount
	 *            Amount worth of units to purchase. It should be equal to or
	 *            greated than minimum_additional_purchase_amount and in
	 *            multiple of purchase_amount_multiplier in the instrument
	 *            master.
	 * @param installmentDay
	 *            If Frequency is monthly, the day of the month (1, 5, 10, 15,
	 *            20, 25) to trigger the order on.
	 * @param instalments
	 *            Number of instalments to trigger. If set to -1, instalments
	 *            are triggered at fixed intervals until the SIP is cancelled.
	 * @param initialAmount
	 *            Amount worth of units to purchase before the SIP starts.
	 *            Should be equal to or greater than minimum_purchase_amount and
	 *            in multiple of purchase_amount_multiplier. This is only
	 *            considered if there have been no prior investments in the
	 *            target fund.
	 ...
	 *             is thrown when there is connection related error.
	 */
	public MFSIP placeMFSIP(String tradingsymbol, String frequency, int installmentDay, int instalments,
			int initialAmount, double amount) throws KiteException, IOException {
		Map<String, Object> params = new HashMap<String, Object>();
		params.put("tradingsymbol", tradingsymbol);
		...
		JSONObject response = new KiteRequestHandler(proxy).postRequest(routes.get("mutualfunds.sips.place"), params,
				apiKey, accessToken);
	 ...
	 *            are triggered at fixed intervals until the SIP is cancelled.
	 * @param sipId
	 *            is the id of the sip.
	 * @return returns true, if modify sip is successful else exception is
	 *         thrown.
	 * @throws KiteException
	 *             is thrown for all Kite trade related errors.
	 * @throws IOException
	 *             is thrown when there is connection related error.
	 */
	public boolean modifyMFSIP(String frequency, int day, int instalments, double amount, String status, String sipId)
			throws KiteException, IOException {
		Map<String, Object> params = new HashMap<String, Object>();
		params.put("frequency", frequency);
		params.put("day", day);
		...
		new KiteRequestHandler(proxy).putRequest(routes.get("mutualfunds.sips.modify").replace(":sip_id", sipId),
	 ...
	 * 
	 * @param sipId
	 *            is the id of mutualfunds sip.
	 * @return returns true, if cancel sip is successful else exception is
	 *         thrown.
	 * @throws KiteException
	 *             is thrown for all Kite trade related errors.
	 * @throws IOException
	...
	public boolean cancelMFSIP(String sipId) throws KiteException, IOException {
		new KiteRequestHandler(proxy).deleteRequest(routes.get("mutualfunds.sip").replace(":sip_id", sipId),
				new HashMap<String, Object>(), apiKey, accessToken);
		return true;
	}
	...
	public List<MFSIP> getMFSIPs() throws KiteException, IOException {
		JSONObject response = new KiteRequestHandler(proxy).getRequest(routes.get("mutualfunds.sips"), apiKey,
				accessToken);
	...
	public MFSIP getMFSIP(String sipId) throws KiteException, IOException {
		JSONObject response = new KiteRequestHandler(proxy)
				.getRequest(routes.get("mutualfunds.sip").replace(":sip_id", sipId), apiKey, accessToken);
	...
	public List<MFHolding> getMFHoldings() throws KiteException, IOException {
		JSONObject response = new KiteRequestHandler(proxy).getRequest(routes.get("mutualfunds.holdings"), apiKey,
				accessToken);
	 ...
	 * @param refreshToken
	 *            is the token received after successful log in.
	 * @throws IOException
	 *             is thrown for connection related errors.
	 * @throws KiteException
	 *             is thrown for Kite trade related errors.
	 */
	...
	private CellProcessor[] getProcessors() {
		CellProcessor[] processors = new CellProcessor[] { new NotNull(new ParseLong()), // instrument_token
	...
	private CellProcessor[] getMfProcessors() {
		CellProcessor[] processors = new CellProcessor[] { new org.supercsv.cellprocessor.Optional(), // tradingsymbol
